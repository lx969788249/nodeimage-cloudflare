...
function setToken(t) { if (t) localStorage.setItem('ni_token', t); else localStorage.removeItem('ni_token'); }
...
function syncSettingsUi() {
  ...
  if (els.deleteDays) els.deleteDays.value = state.settings.deleteDays;
  if (els.daysInputInline) els.daysInputInline.style.display = state.settings.autoDelete ? 'flex' : 'none';
}
...
function renderResults() {
  ...
  state.results.forEach((res) => {
    ...
    imageEl.src = res.thumbUrl || (res.url + '?w=400&f=jpeg&q=' + state.compression.quality);
    imageEl.alt = '图片';
...
function renderHistory() {
  ...
  state.history.items.forEach((img) => {
    ...
    thumbWrap.className = 'thumb-wrap';
    const imageEl = document.createElement('img');
    imageEl.src = img.thumbUrl || (img.url + '?w=400&f=jpeg&q=' + state.compression.quality);
    imageEl.alt = '图片';
    imageEl.loading = 'lazy';
    thumbWrap.appendChild(imageEl);
...
async function loadHistory(page = 1) {
  ...
  try {
    const res = await fetch(`/api/images?page=${page}&limit=12`, { headers: authHeaders() });
    if (res.status === 401) {
      toggleAuthModal(true);
      return;
    }
...
function openImageModal(url) {
  els.modalImage.src = url + '?w=1200&f=jpeg&q=' + state.compression.quality;
...
async function regenerateApiKey() {
  ...
  try {
    const res = await fetch('/api/user/regenerate-api-key', { method: 'POST', headers: authHeaders() });
    const data = await res.json();
...
function setupEventListeners() {
  ...
  async function loadBackupSettings() {
    try {
      const res = await fetch('/api/settings/backup', { headers: authHeaders() });
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function testS3() {
    ...
    try {
      ...
      const res = await fetch('/api/settings/backup/test-s3', { method: 'POST', headers: authHeaders() });
      const data = await res.json();
      if (res.ok) {
  ...
  async function testWebhook() {
    ...
    try {
      ...
      const res = await fetch('/api/settings/backup/test-webhook', { method: 'POST', headers: authHeaders() });
      const data = await res.json();
      if (res.ok) {
  ...
  async function triggerBackup() {
    ...
    let hasRemote = false;
    try {
      const st = await fetch('/api/backup/status', { headers: authHeaders() });
    ...
    try {
      const res = await fetch('/api/backup/auto', { method: 'POST', headers: authHeaders() });
      const data = await res.json();
      if (res.ok) {
...
async function loadBrandingFromServer() {
  try {
    const res = await fetch('/api/settings/branding', { headers: authHeaders() });
